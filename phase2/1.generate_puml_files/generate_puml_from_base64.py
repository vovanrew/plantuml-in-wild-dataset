...
Decodes base64-encoded PlantUML content and creates .puml files

Part of Phase 2: File Preparation
...
Examples:
  # Basic usage
  python3 generate_puml_from_base64.py plantuml_content.txt

  # Custom output directory and batch size
  python3 generate_puml_from_base64.py input.txt -o ./output -b 5000

  # Enable batching (split into subdirectories)
  ...
  python3 generate_puml_from_base64.py input.txt -v
    ...
    parser.add_argument(
        "input",
        help="Input file with PlantUML content (format: blob_id;file_path;base64_content per line)"
    )
    parser.add_argument(
        "-o", "--output-dir",
        type=Path,
    ...
    parser.add_argument(
        "--batch",
        action="store_true",
        help="Split output into batch subdirectories (default: all files in one directory)"
    )
    parser.add_argument(
        "-v", "--verbose",
        action="store_true",
            ...
            for line_num, line in iterator:
                ...
                if not args.dry_run:
                    if create_puml_file(batch_dir, blob_id, content, file_path):
                        ...
                        metadata[blob_id] = {
                            "file_path": file_path,
                            "batch": current_batch if args.batch else None,
