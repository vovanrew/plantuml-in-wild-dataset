...
Original files with multiple diagrams will be moved to phase3/multi_diagram_originalss directory.
"""
...
def split_puml_file(file_path, output_dir, multi_diagram_originals_dir, dry_run=False):
    """
    ...
    Args:
        ...
        multi_diagram_originals_dir: Directory where original multi-diagram files will be moved
    ...
    if dry_run:
        ...
        print(f"  Would move original to: multi_diagram_originalss/{file_path.name}")
        return len(diagrams)
    ...
    # Move original file to multi_diagram_originals directory
    multi_diagram_originals_dir.mkdir(exist_ok=True)
    dest_path = multi_diagram_originals_dir / Path(file_path).name
...
def main():
    ...
    parser.add_argument(
        'puml_dir',
        type=Path,
        help='Directory containing .puml files to process'
    )
    parser.add_argument(
        'multi_diagram_originals_dir',
        type=Path,
        help='Directory to move original multi-diagram files to'
    )
    parser.add_argument(
        '--dry-run',
        action='store_true',
    ...
    multi_diagram_originals_dir = args.multi_diagram_originals_dir
    ...
    if not args.dry_run:
        print(f"Multi-diagram files will be moved to: {multi_diagram_originals_dir}")
    print("-" * 60)
    ...
    for puml_file in sorted(puml_dir.glob("*.puml")):
        diagram_count = split_puml_file(puml_file, puml_dir, multi_diagram_originals_dir, dry_run=args.dry_run)
    ...
    if not args.dry_run:
        print(f"  Original files {moved_verb} to: {multi_diagram_originals_dir}")
