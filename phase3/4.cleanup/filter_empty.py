#!/usr/bin/env python3
"""Remove empty diagrams (content_lines == 0) from the dataset.

Reads the classification JSON (with line metrics already added),
identifies entries with content_lines == 0, deletes corresponding
.puml and .png files, and writes a filtered classification JSON.

Usage:
    python3 filter_empty.py \
...
def main():
    parser = argparse.ArgumentParser(
        description="Filter empty diagrams (content_lines == 0) from the dataset."
    )
    parser.add_argument(
        "--classification", required=True, type=Path,
        help="Input classification JSON with line metrics (not modified)",
    )
    parser.add_argument(
    ...
    for fname, entry in classifications.items():
        if entry.get("content_lines") in (0, None):
            to_remove[fname] = entry
        else:
            to_keep[fname] = entry

    print(f"Total entries: {total_before}")
    print(f"Entries to remove (content_lines == 0 or missing): {len(to_remove)}")
    print(f"Entries to keep: {len(to_keep)}")

    if to_remove:
        print(f"\nEmpty entries by primary_type:")
