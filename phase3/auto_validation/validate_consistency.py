...
Usage:
    ...
    python3 validate_consistency.py -i uml_classifications.json -s element_connection_counts.json -o report.json
    python3 validate_consistency.py -i uml_classifications.json -s element_connection_counts.json --severity-filter error --only-inconsistent
"""
...
def detect_flags(
    ...
    # Confidence-based checks only when confidence is available
    if has_confidence:
...
def validate_file(entry: Dict[str, Any], threshold: float) -> Dict[str, Any]:
    ...
    Args:
        entry: Merged entry containing primary_type and elements
    ...
    confidence = confidence if has_confidence else 0.0
...
def load_stats_jsonl(stats_path: Path) -> Dict[str, Dict[str, Any]]:
    """
    Parse JSONL stats file into a dict mapping filename -> {elements, elements_total}.
    """
    stats = {}
...
def process_analysis(input_path: Path, stats_path: Path, threshold: float) -> Dict[str, Any]:
    """
    Main processing function - merge classification + element data and validate.

    Args:
        input_path: Path to classification JSON (with 'classifications' key)
        stats_path: Path to element_connection_counts JSONL
        threshold: Minimum consistency score to consider valid
    ...
    print(f"Loading classifications from {input_path}...", file=sys.stderr)
    with open(input_path, 'r', encoding='utf-8') as f:
        data = json.load(f)

    classifications = data.get('classifications', {})
    print(f"Found {len(classifications)} classified files", file=sys.stderr)

    print(f"Loading element stats from {stats_path}...", file=sys.stderr)
    element_stats = load_stats_jsonl(stats_path)
    print(f"Found {len(element_stats)} element stats entries", file=sys.stderr)

    # Merge: inject elements into classification entries
    source_results = {}
    for filename, entry in classifications.items():
        merged = dict(entry)
        stats = element_stats.get(filename, {})
        merged['elements'] = stats.get('elements', {})
        source_results[filename] = merged

    print(f"Merged {len(source_results)} files to validate", file=sys.stderr)
...
Examples:
  python3 validate_consistency.py -i uml_classifications.json -s element_connection_counts.json
  python3 validate_consistency.py -i uml_classifications.json -s element_connection_counts.json -o report.json
  python3 validate_consistency.py -i uml_classifications.json -s element_connection_counts.json --consistency-threshold 0.6
  python3 validate_consistency.py -i uml_classifications.json -s element_connection_counts.json --severity-filter error --only-inconsistent
...
Output:
  ...
  - Confusion matrix (classified type vs suggested type)
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Classification JSON with primary_type per file"
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="JSONL file with elements from DiagramStatsExtractor"
    )
    ...
    print("=" * 60, file=sys.stderr)
    print(f"Classifications: {args.input}", file=sys.stderr)
    print(f"Element stats:   {args.stats}", file=sys.stderr)
    print(f"Output:          {args.output}", file=sys.stderr)
    print(f"Threshold:       {args.consistency_threshold}", file=sys.stderr)
    print("=" * 60, file=sys.stderr)

    output = process_analysis(args.input, args.stats, args.consistency_threshold)
